...
class TranslationModelTest(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.translation_en = Translation.objects.create(
            parent=cls.parent, language="en"
        )
        cls.translation_fr = Translation.objects.create(
            parent=cls.parent, language="fr"
        )
    ...
    def test_get_all_translations(self):
        self.assertEqual(
            self.translation_en.get_all_translations(),
            list([self.translation_en, self.translation_fr]),
        )

    def test_get_other_translations(self):
        self.assertEqual(
            self.translation_en.get_other_translations(), list([self.translation_fr])
        )
